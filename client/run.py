...
def make_and_log_request(port):
    start = time.time()
    response = requests.get('http://127.0.0.1:' + port)
...
def run_one_at_a_time():
    plots = []
    for port in ('8888', '8887', '8881'):
        plot_list = []
        plots.append(plot_list)
        for i in range(200):
            size, rt_time = make_and_log_request(port)
